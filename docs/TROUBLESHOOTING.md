# Troubleshooting — IT-Stack REDIS
...
4. Verify port is not already in use: ss -tlnp | grep 6379
...
```bash
# Test connectivity
psql -h lab-db1 -U redis_user -d redis_db -c '\conninfo'

# Check pg_hba.conf allows connection from lab-db1
```

### Performance issues

1. Check resource usage: docker stats it-stack-redis
...
| Log | Path |
|-----|------|
| Application | docker compose logs redis |
